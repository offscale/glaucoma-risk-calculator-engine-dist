...
export const ethnicities_pretty = (ethnicities) => ethnicities.map(study => (study_name => `${study_name}: ${study[study_name].join(', ')}`)(Object.keys(study)[0]));
export const s_col_to_s = (s) => s.slice(0, s.indexOf(':'));
export const in_range = (range, num) => {
...
export const lowest_range = (ranges) => ranges.reduce((prevNum, currentValue) => {
...
export const preprocess_studies = (risk_json) => {
    ...
    Object.keys(risk_json.studies).forEach(study_name => {
        if (risk_json.studies[study_name].hasOwnProperty('age')) {
            const sr = sort_ranges(Object.keys(risk_json.studies[study_name].age));
            // Lower bound
            if (sr[0][0] !== '<') {
        ...
        if (risk_json.studies[study_name].hasOwnProperty('agenda')) {
            const all_genders_seen = uniq(risk_json.studies[study_name].agenda.map(agenda => agenda.gender));
            let gendersAssigned = 0;
            all_genders_seen.forEach(gender => {
                const sr = sort_ranges(risk_json.studies[study_name].agenda.filter(agenda => agenda.gender === gender).map(agenda => agenda.age));
                if (sr.length) {
                    // Lower bound
...
export const sort_ranges = (ranges) => ranges.sort((a, b) => {
    if (a[0] === '<')
        return -1;
    else if (a[0] === '>')
...
export const risk_from_study = (risk_json, input) => {
    ...
    const out = isArray(study_vals) ? study_vals.filter(o => study.expr[0].filter.every(k => k === 'age' ? in_range(o.age, input.age) : input.hasOwnProperty(k) ? o[k] === input[k] : true))[study.expr[0].take > 0 ? study.expr[0].take - 1 : 0]
        : study_vals[ensure_map(study.expr[0].type) && Object.keys(study_vals).filter(k => in_range(k, input[study.expr[0].key]))[study.expr[0].take - 1]];
    if (!out)
...
export const familial_risks_from_study = (risk_json, input, warn = true) => {
...
export const combined_risk = (familial_risks_from_study_l, risk_from_studies) => math.add(familial_risks_from_study_l.map(r => math.multiply(math.divide(r, 100), risk_from_studies)).reduce(math.add), risk_from_studies);
export const risks_from_study = (risk_json, input) => {
    if (isNullOrUndefined(risk_json))
        throw TypeError('`risk_json` must be defined');
    else if (isNullOrUndefined(input))
        throw TypeError('`input` must be defined');
...
export const place_in_array = (entry, a) => {
...
export const pos_in_range = (ranges, num) => {
...
export const list_ethnicities = (risk_json) => {
    if (isNullOrUndefined(risk_json))
        throw TypeError('`risk_json` must be defined');
